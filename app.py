...
def interactive_template_with_address_buttons(rcvr, body, message):
    ...
    try:
        response = requests.post(url, data=payload.encode('utf-8'), headers=headers, verify=False)
        response.raise_for_status()
        print(f"Address buttons response: {response.text}")
        savesentlog(rcvr, response.text, response.status_code, "address_confirmation")
        return response.text
    except requests.RequestException as e:
        print(f"Interactive address buttons failed: {e}, Response: {response.text if 'response' in locals() else 'No response'}")
        return None
...
#     if resp1.lower() in [g.lower() for g in greeting_word]:
#         return False
    
#     # Allow alphabetic characters, spaces, and Unicode for non-Latin scripts (e.g., Hindi)
#     if not re.match(r'^[\u0900-\u097Fa-zA-Z\s]+$', resp1):
#         return False
    
#     # Trim and split input into words
#     resp1 = resp1.strip()
#     words = resp1.split()
    
#     # Restrict to 1–3 words for names
#     if len(words) < 1 or len(words) > 3:
#         return False
    
#     # Reject common question words or phrases
#     question_words = {'how', 'what', 'why', 'are', 'is', 'do', 'does', 'can', 'will'}
#     if any(word.lower() in question_words for word in words):
#         return False
    
#     # Ensure each word is at least 2 characters
#     for word in words:
#         if len(word) < 2:
#             return False
...
def Get_Message():
    ...
    try:
        ...
        if (msg_type == 'text' or msg_type == 'interactive' or msg_type == 'order') and len(frm) == 12:
            if resp1 in greeting_word:
                if result is None:  # New user
                    ...
                    if profile_name and is_valid_name(profile_name):  # Validate WhatsApp profile name
                        ...
                        send_message(frm, wl.format(name=name), 'pincode')  # Send wl with pincode prompt
                    else:
                    ...
                    if name:  # If name exists in database
                        ...
                        send_message(frm, r2.format(name=name), 'pincode')  # Use r2 for existing users
                    else:
            ...
            # Existing pincode handling logic (unchanged)
            if camp_id == '1':
                ...
                # Address confirmation block for existing users
                    ...
                    if resp1 == "6":  # Proceed with existing address
                        print(f"User {frm} chose to proceed with existing address: {address}")
                        ...
                        print(f"User {frm} chose to enter new address")
                            ...
                            try:
                                ...
                                if camp_id == '1':
                                    ...
                                    if previous_address:
                                        ...
                                        print(f"Sending address confirmation to {frm} with previous address: {previous_address[0]}")
                                        interactive_template_with_address_buttons(frm, address_message, "address_confirmation")
                                    else:
                                        print("No previous address found for existing user, asking for new address")
                                        send_message(frm, m3, "ask_address")
                                else:
                                    print("New user, asking for new address")
                                    send_message(frm, m3, "ask_address")
                            except Exception as e:
